...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIsScheduled]) {
		...
		[[self readStream] scheduleInRunLoop:[NSRunLoop currentRunLoop] forMode:/*[self runLoopMode]*/NSRunLoopCommonModes];
...
+ (void)runRequests
{
	...
	while (runAlways) {
		NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
        CFRunLoopRunInMode(kCFRunLoopDefaultMode, 1.0e10, true);//崩溃点
		[pool drain];
	}
