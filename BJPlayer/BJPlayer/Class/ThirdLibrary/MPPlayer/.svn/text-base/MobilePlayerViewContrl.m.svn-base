...
-(void)AddTopViews
{
    ...
    self.myCourseBackView=[[UIView alloc] initWithFrame:CGRectMake(self.myMoviePlayer.view.frame.size.width-272-GB_HorizonDifference, 0, 272+GB_HorizonDifference,320+GB_HorizonDifference)];
    ...
    self.mybackBtn.frame=CGRectMake(232+GB_HorizonDifference, 20, 40, 40);
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (startPoint.y > self.myMovieControlsView.height - self.myMovieControlsView.bottomViewBar.height && startPoint.y < self.myMovieControlsView.height - self.myMovieControlsView.bottomViewBar.height + 30) {
        ...
        if (fabsf(changeX) > fabsf(changeY))
        {
            ...
            [self.hintView tinkerUp:progressTime+playingTime With:[GlobalData GetInstance].NewmoviePlayer.duration];
        }
        else
        {
            self.hintView.hidden=NO;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (!self.myMovieControlsView.isHorizontal) {
        ...
        if (startPoint.y > self.myMovieControlsView.height - self.myMovieControlsView.bottomViewBar.height && startPoint.y < self.myMovieControlsView.height - self.myMovieControlsView.bottomViewBar.height + 30 && startPoint.x < self.myMovieControlsView.bottomViewBar.width - 60) {
...
-(void)UUpdateProgrss;
{
    ...
    float curTime=[GlobalData GetInstance].NewmoviePlayer.currentPlaybackTime;//当前播放时间
    float playableTime=[GlobalData GetInstance].NewmoviePlayer.playableDuration;//缓冲时间
    float allTime=[GlobalData GetInstance].NewmoviePlayer.duration;//总时长
